...
import { sendReservationNotification } from './notification.service';
...
export const reservationValidationSchema = z.object({
  ...
  date: z.coerce.date()
    .refine((val) => val > new Date(), { message: "Tarih geçerli bir gelecek tarih olmalıdır" })
    .refine((val) => {
      const limitDate = new Date();
      limitDate.setMonth(limitDate.getMonth() + 3);
      return val <= limitDate;
    }, { message: "İleriye dönük en fazla 3 ay için rezervasyon yapılabilir" }),
...
export const createReservation = async (input: ReservationInput): Promise<IReservation> => {
  ...
  // Send WhatsApp notification asynchronously (don't await to avoid blocking)
  console.log(`[Reservation] Created new reservation ${newReservation._id}. Triggering notification...`);
  sendReservationNotification(newReservation)
    .then(() => console.log(`[Reservation] Notification process finished for ${newReservation._id}`))
    .catch(err => console.error(`[Reservation] Notification failed for ${newReservation._id}`, err));
